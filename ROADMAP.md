...
| Follow-up source/status handling | `[x]` | Source provenance follow-ups use the in-memory Research Guard decision buffer instead of literal follow-up searches. v0.6.3 also detects direct Research Guard status/diagnostic prompts before source-follow-up matching and prevents bare `research guard` from being treated as a source question. |
...
| Status response policy | `[x]` | Tool output is diagnostic-only through schema description and structured status payload; v0.6.3 injects status-v2 diagnostics for direct status prompts when Hermes does not call the tool. |
