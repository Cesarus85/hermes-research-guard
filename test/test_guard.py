...
class ResearchGuardHeuristicTests(unittest.TestCase):
    ...
    def test_route_planning_context_is_injected_from_google_payload(self):
        ...
        self.assertIn("Fahrtzeit mit aktueller Verkehrslage", result["context"])
        self.assertIn("statische Dauer ohne aktuelle Verkehrslage", result["context"])
        self.assertIn("Fahrtzeit-Label-Regel", result["context"])
        self.assertIn("Nenne `staticDuration` nicht `typisch`", result["context"])
        self.assertNotIn("ohne/typisch", result["context"])
        ...
        self.assertIn("an groben Route-Samples gefundene Kandidaten", result["context"])
        self.assertNotIn("Kandidaten entlang/nahe der Route", result["context"])
        ...
        self.assertIn("Kandidat mit bestätigten Connector-Daten", result["context"])
        self.assertNotIn("stärker belegter Kandidat", result["context"])
        self.assertIn("Schreibe nicht `Empfehlung`, `ideal`, `beste Option`, `hohe Verfügbarkeit`, `der ID.7 lädt hier schnell`", result["context"])
        self.assertIn("Reihenfolge-Verbot", result["context"])
        self.assertIn("`erster Ladestopp`, `zweiter Stopp`", result["context"])
        self.assertIn("`Zwei-Stopp-Strategie`", result["context"])
        ...
        self.assertIn("an einem groben Suchpunkt entlang der Route gefunden", result["context"])
        self.assertIn("nur grobe Route-Samples entlang der Polyline", result["context"])
        ...
        self.assertIn("Verlaufsketten-Ausgabe-Regel", result["context"])
        self.assertIn("muss die Zeile exakt mit `Geprüfte Verlaufskette:` beginnen", result["context"])
        self.assertIn("Schreibe niemals `Verlauf:` oder `Streckenverlauf:`", result["context"])
        self.assertNotIn("Auf A73 Richtung Nürnberg fahren", result["context"])
        self.assertIn("Maut-/Kosten-Regel", result["context"])
        self.assertIn("Schreibe nicht `wahrscheinlich relevant`, `Vignette nötig`, `Brennermaut`, `vergiss die Vignette nicht`", result["context"])
        ...
        self.assertIn("Geografie-Interpretations-Regel", result["context"])
        self.assertIn("`vor dem Alpenanstieg`, `nach dem Brenner`, `Brenner-Pass relevant`, `italienische Etappe`", result["context"])
    ...
    def test_route_followup_reuses_previous_route_context_without_google(self):
        ...
        self.assertIn("Keine Reihenfolge und keine geografischen Bewertungen wie `vor dem Alpenanstieg`, `nach dem Brenner`, `italienische Etappe`", result["context"])
        self.assertIn("Biete nicht an, ABRP, PlugShare, VW-App", result["context"])
