...
def _format_route_context(payload: dict[str, Any], request: dict[str, Any], model: str | None) -> str:
    ...
    lines = [
        ...
        "Bewertungssprache: Wenn Kandidaten gute Connector-Daten haben, schreibe höchstens `plausibel zu prüfen` oder `Kandidat mit bestätigten Connector-Daten`. Schreibe nicht `Empfehlung`, `ideal`, `beste Option`, `hohe Verfügbarkeit`, `der ID.7 lädt hier schnell` oder ähnliche Optimierungsaussagen.",
        ...
        "Wenn der Nutzer ideale Lade-/Tankstopps verlangt, sage klar: Research Guard liefert an groben Route-Samples gefundene Kandidaten, aber keine echte ABRP-/Live-Ladeplanung oder fahrzeugspezifisch optimierte Stoppreihenfolge.",
        ...
        "Wenn du Ladepunkte nennst, formuliere sie als Kandidaten, die an einem groben Suchpunkt entlang der Route gefunden wurden, nicht als garantierte funktionierende Stopps oder direkt auf der Route liegende Stationen.",
        "Wenn du Tankstellen nennst, formuliere sie als Kandidaten, die an einem groben Suchpunkt entlang der Route gefunden wurden, nicht als garantierte Kraftstoffverfügbarkeit, Preisangabe oder direkt auf der Route liegende Stationen.",
        ...
        "Stopppositions-Regel: `Routenpunkt`, `route_position` und `route_progress_percent_approx` sind nur grobe Route-Samples entlang der Polyline. Verwende sie nicht als exakte Stoppreihenfolge, Etappe, Segmentdistanz oder Beweis, dass ein Ladepunkt direkt auf der Route liegt.",
...
def _format_route_followup_context(decision: dict[str, Any], user_message: str, model: str | None) -> str:
    ...
    lines = [
        ...
        "Wenn Kandidaten gute Connector-Daten haben, schreibe höchstens `plausibel zu prüfen` oder `Kandidat mit bestätigten Connector-Daten`. Schreibe nicht `Empfehlung`, `ideal`, `beste Option`, `hohe Verfügbarkeit`, `der ID.7 lädt hier schnell` oder ähnliche Optimierungsaussagen.",
